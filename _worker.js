...
export default {
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (!upgradeHeader || upgradeHeader !== 'websocket') {
          ...
          default:
            ...
            try {
              ...
              if (proxyResponse.status !== 200) {
                ...
                const proxyUrl = `https://${proxyIP}${url.pathname}${url.search}`;
        ...
        return วเลสOverWSHandler(request);
      }
    } catch (err) {
      console.error(`Error during request handling: ${err}`);
      return new Response(`Error during request handling: ${err}`, {
        status: 500,
        statusText: 'Internal Server Error',
      });
    } finally {
  ...
  // Other functions should be defined within this scope
  async uuid_validator(request) {
  ...
  // Other functions...
  async วเลสOverWSHandler(request) {
    // WebSocket logic here
  ...
  // Other functions...
  async processวเลสHeader(วเลสBuffer, userID) {
  ...
  // Other functions...
  async remoteSocketToWS(remoteSocket, webSocket, วเลสResponseHeader, retry, log) {
    // remote--> ws
